...
## Which partner should we focus on with lower target achievement in test pos --------
  ...
  df_msd %>%
    ...
    calc_achievement() %>%
    mutate(hts_volume = ifelse(targets > 500, "High", "Low")) %>%
    arrange(achievement) %>%
...
## SNU decline in testing but increase in positivity over last few qtrs. ----------------------
  ...
  df_hts_snu1 <- df_msd %>%
    ...
    group_by(snu1) %>%
    arrange(period) %>%
    mutate(HTS_TST_trend = (HTS_TST - lag(HTS_TST)) / lag(HTS_TST),
           pos_trend = (positivity - lag(positivity)/ lag(positivity))
           ) %>%
    ungroup() %>%
    mutate(flag_up_down = (HTS_TST_trend < 0 & pos_trend > 0),
  ...
  df_hts_snu1 %>%
    ...
    geom_col(aes(y = HTS_TST_POS), fill = tango, width = 0.5,
    ...
    si_style_ygrid() +
    labs(title = "HTS_TST COMPARED TO HTS_TST_POS RESULTS ACROSS SNU1S",
         subtitle = "No Snu1s have declining testing and increasing positivity",
...
## Which age bands have had two or more quarters with declining net new--------
  ...
  df_tx_nn <- df_msd %>%
    ...
    reshape_msd() %>%
    group_by(age_2019) %>%
    mutate(tx_nn_decline = value < 0,
           tx_nn_count = sum(tx_nn_decline)) %>%
    ungroup()

  # Label the periods on the graph
  period_labels <- df_tx_nn %>% distinct(period) %>% pull()
  ...
  df_tx_nn %>%
    ...
    labs(title = "AGES 45 AND ABOVE LOGGED FEWER THAN TWO QUARTERS OF TREATMENT LOSS",
         x = NULL, y = "TX_NET_NEW",
         caption = glue("{meta$caption}")) +
...
## Are any sub-national units (SNUs) lagging behind 6 MO MMD to a majority share --------
  ...
  df_mmd <- df_msd %>%
    ...
    left_join(df_tx) %>%
    mutate(mmd_6mo_share = tx_mmd / tx_curr,
           mmd_order = case_when(
...
# Do we see any differences in viral load coverage (VLC) by age and sex in current year
  ...
  df_vlc <- df_msd %>%
    filter(age_2019 %ni% c("Unknown Age", "Newer Age Band"),
   ...
   gophr::clean_indicator() %>% # Affixes an N or D to TX_PVLS stub
    ...
    # Plot as dot plot --> flipped at last minute
    df_vlc %>%
    filter(period == meta$curr_pd) %>%
    ...
    geom_point(size = 6) +
    geom_point(size = 6, stroke = 0.9, shape = 1, color = "white") +
    ...
    scale_color_identity(labels = c("Female", "Male"),
                         ...
                         name = "Sex") +
    si_style() +
    labs(title = "ADULT MEN APPEAR TO HAVE BETTER VIRAL LOAD COVERAGE THAN ADULT WOMEN",
         caption = glue("{meta$caption}"),
         x = NULL,
