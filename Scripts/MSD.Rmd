...
```{r}
data <- read_psd("~/Github/training_day/Data/MER_Structured_TRAINING_Datasets_PSNU_IM_FY59-62_20240816_v1_1.zip")
```
...
normalized_summary_table_q1 <- summary_table_q1 %>%
  ...
  calc_achievement() %>%
  mutate(
    score = (normalized_targets + normalized_cumulative) * (1 - achievement)*10
  ) %>%
  arrange(desc(score))
```
...
total_percentage_both_categories <- paste0(
  round(((cumulative_txnew_cd4_comparison$total_cumulative_less_than_200 + cumulative_txnew_cd4_comparison$total_cumulative_greater_than_or_equal_200) / cumulative_txnew_cd4_comparison$total_tx_new) * 100, 1),
  "%"
)
```
...
```{r, echo=FALSE}
print(unique(net_new_declining$ageasentered))
```
...
q7_fy <- data %>%
  ...
  summarise(total = sum(cumulative, na.rm = TRUE), .groups = "drop") %>%
  pivot_wider(
    names_from = otherdisaggregate,
    values_from = total,
    values_fill = list(total = 0) # Ensures missing categories are treated as 0
  ) %>%
  left_join(df_tx, by = join_by(snu1)) %>%
  mutate(
    ...
    share_denom_tx_curr = paste0(round((`ARV Dispensing Quantity - 6 or more months` / tx_curr) * 100, 1), "%"),
    share_denom_arv = paste0(round((`ARV Dispensing Quantity - 6 or more months` / total_arv) * 100, 1), "%")
  ) %>%
  select(snu1, `ARV Dispensing Quantity - 3 to 5 months`,
  `ARV Dispensing Quantity - 6 or more months`,
  `ARV Dispensing Quantity - Less than 3 months`, share_denom_tx_curr, share_denom_arv)
```
...
df_tx_qtr <- data_long %>%
  ...
  group_by(quarter) %>%
  count(snu1, wt = value, name = "tx_curr")
...
q7_fy_qtr <- data_long %>%
  ...
  group_by(snu1, quarter, otherdisaggregate) %>%
  ...
  left_join(df_tx_qtr, by = join_by(snu1, quarter)) %>%
  mutate(
    less_than_6_months = `ARV Dispensing Quantity - 3 to 5 months` + `ARV Dispensing Quantity - Less than 3 months`,
    total_arv = less_than_6_months + `ARV Dispensing Quantity - 6 or more months`,
    share_denom_tx_curr = paste0(round((`ARV Dispensing Quantity - 6 or more months` / tx_curr) * 100, 1), "%"),
  ...
  select(quarter, snu1, `ARV Dispensing Quantity - 3 to 5 months`,
  `ARV Dispensing Quantity - 6 or more months`,
  `ARV Dispensing Quantity - Less than 3 months`, share_denom_tx_curr, share_denom_arv)
```
...
q7_fy_qtr
```
To undersand the difference between the two share values, we look at who is providing ARVDispense. It looks like MMD data is provided (Total Numerator = Age/Sex/ARVDispense/HIVStatus)for all in Midwest and Pacific Coast, but not for Northwest and South Atlantic.  
```{r}
  data %>%
    filter(
      indicator == "TX_CURR",
    ...
    mutate(diff = .[[3]] - .[[4]]) %>%
  select(snu1, diff, `Age/Sex/ARVDispense/HIVStatus`, `Total Numerator`)
```
...
```{r, echo=FALSE}
print(results_q8 %>%
  mutate(VLC_Percentage = paste0(round(VLC_Percentage, 1), "%")))
```
